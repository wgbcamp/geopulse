...
import { RoutingDispatchContext } from '../../context/Routing';

export const NewHeader = ({ props }: any) => {

    const page = useContext(RoutingContext);
    const dispatch = useContext(RoutingDispatchContext);
        ...
        <div className={`flex fixed top-0 z-3 w-full flex-wrap 2xl:flex-nowrap ${dataOptions || menuOptions ? '' : 'overflow-hidden h-14.75'} lg:h-[unset]`}>
            <div className={`flex flex-col ${menuOptions ? 'z-2 h-73.75' : '-z-10 h-0 overflow-hidden'} w-full lg:w-58 bg-(--accentdarkblue-90) lg:bg-[unset] absolute shadow-xl/40`}>
                ...
                <div className='pl-7 bg-(--accentdarkblue-90)'>
                    <div className='flex items-center w-full h-14.75 text-white cursor-pointer' onClick={ () => {props.setView('About'); setMenuOptions(false); updateURL("/about"); } }>About</div>
                    <div className='flex items-center w-full h-14.75 text-white cursor-pointer' onClick={ () => {props.setView('DataMethodology'); setMenuOptions(false); updateURL("/datamethodology"); } }>Data & Methodology</div>
            ...
            <div className='h-14.75 flex grow lg:grow-0 lg:w-70'>
                <Card className={`shadow-none rounded-none grow p-0 h-full flex flex-col items-center justify-end gap-0 hover:bg-white transition-colors duration-200 ${page.pageStatus == "/compare" || page.pageStatus == "/grid" ? 'bg-white' : 'bg-(--accentblue-30)'} border-0`}>
            ...
            {(page.pageStatus === "/compare" || page.pageStatus === "/grid") ?
                <div className={`flex flex-col lg:flex-row 2xl:flex-row w-full  ${dataOptions ? 'h-full' : 'h-0'} lg:h-14.75 overflow-hidden`}>
