    ...
    return (
        <div className={`flex fixed top-0 z-3 w-full flex-wrap 2xl:flex-nowrap ${dataOptions || menuOptions ? '' : 'overflow-hidden h-14.75'} lg:h-[unset]`}>
            ...
            <div className='h-14.75 flex flex-wrap lg:flex-nowrap lg:w-40'>
                <Card className={`rounded-none p-0 flex flex-col items-center justify-center gap-0 hover:bg-white transition-colors duration-200 ${location.pathname == "/eventtracking" ? 'bg-white' : 'bg-(--accentblue-30)'} border-0`}>
                    <div className='h-full flex flex-col justify-end w-40'>
                        ...
                        <div className="flex flex-row h-[35px] items-center justify-center cursor-pointer">
                            ...
                            <Link to="/eventtracking" activeOptions={{ exact: true }} className={`text-sm font-bold text-end flex items-center pl-1 ${location.pathname == "/eventtracking"  ? 'text-(--orange)' : 'text-black'}`} onClick={() => {actions?.setView("Event tracking"); }}>Event Tracking</Link>
                        </div>
                    </div>
